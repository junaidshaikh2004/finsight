...
// Attaches the bearer token to a supertest request the same way the
// frontend's API client does — no cookie jar involved anywhere in this file.
function auth(req) {
...
beforeAll(async () => {
  const signupRes = await request(app)
    ...
    .send({ name: 'Expense Tester', email: testEmail, password: 'password123' });
  token = signupRes.body.token;

  const categoriesRes = await auth(request(app).get('/api/categories'));
...
describe('Expenses API', () => {
  ...
  test('rejects a non-positive amount', async () => {
    const res = await auth(
      request(app).post('/api/expenses').send({ category_id: categoryId, amount: -5, date: '2026-09-05' })
    );
  ...
  test('rejects a category that does not belong to the user', async () => {
    const res = await auth(
      request(app).post('/api/expenses').send({ category_id: 999999, amount: 10, date: '2026-09-05' })
    );
  ...
  test('lists expenses filtered by month, including the one just created', async () => {
    const res = await auth(request(app).get('/api/expenses?month=2026-09'));
